      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-3xl font-extrabold text-gray-900">{t("orders_order_label")} {order.public_order_id}</h1>
            <span className="px-3 py-1 bg-emerald-100 text-emerald-800 text-xs font-bold rounded-lg uppercase">
              {order.order_status}
            </span>
          </div>
          <p className="mt-1 text-xs text-gray-500">
            {t("orders_placed_on")} {new Date(order.created_at).toLocaleString()} &bull; {t("orders_store")}{" "}
            <span className="font-semibold text-gray-800">{order.store_name || t("orders_flagship")}</span>
          </p>
        </div>

        <div className="text-right">
          <p className="text-xs text-gray-500 font-semibold uppercase">{t("orders_total_amount")}</p>
          <p className="text-3xl font-extrabold text-gray-900">{formatPrice(Number(order.total_price), currency)}</p>
      ...
      {shipment && (
        <div className="bg-emerald-900 text-white rounded-3xl p-6 shadow-lg mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-bold text-base">{t("orders_dispatched_via")} {shipment.courier_name || "Express Delivery"}</h3>
              <p className="text-xs text-emerald-200">
                {t("orders_tracking_number")} <span className="font-mono font-bold text-white">{shipment.tracking_number}</span>
              </p>
            </div>
          </div>
                ...
                <div key={item.id} className="flex gap-4 p-4 rounded-2xl border border-gray-100 bg-gray-50/50">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {t("orders_quantity")}{" "}
                      <span className="font-semibold text-gray-800">{item.quantity_packages} {item.package_name}</span>
                    </p>
                    <p className="text-xs font-bold text-emerald-700 mt-2">{formatPrice(Number(item.price), currency)} {t("orders_each")}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
            ...
            <div className="text-xs text-gray-600 space-y-1">
              ...
              <p className="font-mono text-gray-500 mt-2">{t("orders_phone")} {order.phone}</p>
              <p className="text-gray-500">{t("orders_email")} {order.customer_email}</p>
            </div>
          </div>
          ...
          {order.order_status === "Delivered" && !order.received_confirmed_at && (
            <div className="bg-emerald-50 rounded-3xl p-6 border border-emerald-200">
              <h4 className="font-bold text-emerald-950 text-sm mb-2">{t("orders_confirm_receipt")}</h4>
